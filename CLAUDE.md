...
M3.5 進行中。已完成：M1、M2 全部，M3 的階段系統與王，以及無盡循環、
五種敵人（蟲群／裝甲兵／護盾兵／疾行群／王）與幾何護盾。
尚未做：分享碼、音效、第二把武器。
...
src/
  ...
  gameover.ts        死亡結算畫面
  records.ts         本機成績（localStorage）
  advice.ts          **依腳本結構與死法給建議 —— 全遊戲唯一主動教學的地方**
  slots.ts           腳本記憶槽（打贏王解鎖，跨局保留）
  main.ts            組裝與主迴圈
tools/
  sim.ts             無頭模擬器（平衡量測）
