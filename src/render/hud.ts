 ...
 * 迴圈就變成一道**看得見的行進波**。
 *
 * 0.2 是實際比對 0／0.12／0.2／0.3／0.45 之後選定的。散彈手一輪 418ms
 * ＝執行 83ms＋冷卻 350ms：0.2 秒剛好涵蓋整段執行，又能在下一輪開始前
 * 退乾淨留出黑暗間隙 —— 脈衝的節奏於是就是攻擊週期本身。太長（0.45）
 * 畫面永遠不會全暗，變成一直亮著的糊團；太短則來不及讀。
 *
 * 這個值只影響顯示，完全不動遊戲速度 —— 可讀性與遊戲性不必互相犧牲。
 * M1 接上 Blockly 後積木面積大很多，屆時可能要重新校一次。
 */
const HEAT_DECAY = 0.2;
